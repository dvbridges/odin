class MainPage {
	constructor() {
		this._projectN =
			localStorage.getItem("projectN") === undefined
				? 0
				: Number(localStorage.getItem("projectN"));
	}
	...
	_makeProject(project) {
		...
		projectBoxTitle.innerHTML =
			project.title === undefined ? `Project ${this._projectN}` : project.title;
		projectBoxDesc.innerHTML =
			project.desc === undefined ? "Description" : project.desc;
	...
	_makePlan(plan) {
		...
		if (plan) {
			let checkContainer = this._makeElement("div", [
				"plan-element",
				"plan-checkbox-container"
			]);
			let titleContainer = this._makeElement("div", [
				"plan-element",
				"project-title-container"
			]);
			let descContainer = this._makeElement("div", [
				"plan-element",
				"project-desc-container"
			]);
			let statusContainer = this._makeElement("div", [
				"plan-element",
				"project-title-container"
			]);
			let dueDateContainer = this._makeElement("div", [
				"plan-element",
				"project-title-container"
			]);
			let priorityContainer = this._makeElement("div", [
				"plan-element",
				"project-title-container"
			]);
			...
			planDetails.style.background =
				plan.status === "Finished" ? "silver" : priorityCols[plan.priority];
